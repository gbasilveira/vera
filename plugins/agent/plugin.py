...
Copy this directory to plugins/agent/ and rename accordingly.
...
class AgentPlugin(VeraPlugin):
    """Minimal working plugin. Replace all 'template'/'agent' references."""
    name = "agent"
    version = "1.0.0"

    def register_tools(self, kernel: VeraKernel) -> None:
        kernel.register_tool(
            tool_name="agent.do_thing",
            fn=self._do_thing,
